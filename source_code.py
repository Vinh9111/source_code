...
print(f"""
\033[91m----------------------------------------
\033[1;32m◇ Lấy Key Tại: https://web1s.org/Keyv5 ◇
\033[91m----------------------------------------
""")
def authenticate(password):
    if password == "100088521904540":
        return True
    else:
        return False
def login():
    ...
    while attempts > 0:
        ...
        try:
            if authenticate(password):
                ...
                print("Đăng nhập không thành công. Vui lòng kiểm tra lại tên người dùng và mật khẩu.")
                attempts -= 1
                print(f"Số lần thử đăng nhập còn lại: {attempts}")
        except Exception as e:
            print(f"Lỗi xảy ra: {e}")
        if attempts == 0:
            print("Hết số lần thử đăng nhập. Chương trình sẽ thoát.")
            exit()
            break
login()
def install_colorama():
    try:
        ...
        print("Không Thành Công.", e.stderr.decode("utf-8"))
install_colorama()
def install_progressbar2():
    try:
        subprocess.run(["pip", "install", "progressbar2"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        print("Đang chuẩn bị.")
    except subprocess.CalledProcessError as e:
        print("Không Thành Công.", e.stderr.decode("utf-8"))
install_progressbar2()
import os
import time
...
def loading_animation():
    ...
    bar = progressbar.ProgressBar(widgets=widgets, max_value=20).start()
    for i in range(20):
        time.sleep(0.1)
        bar.update(i + 1)
...
\033[1;32m║ ░▒▓█▓▒░        ░▒▓█▓▒░  ░▒▓█▓▒░░▒▓█▓▒░ 
\033[1;32m║ ░▒▓█▓▒░        ░▒▓█▓▒░   ░▒▓██████▓▒░  
\033[1;32m║ ➢ SNEPS Social Network Express ⚜️     
\033[1;32m║ ➢ FACEBOOK: https://www.facebook.com/chongiukhann?mibextid=ZbWKwL
\033[1;32m║ ➢ GROUP ZALO: https://zalo.me/g/dmqedp806    
\033[1;32m║ ➣ Version: 0.1.3
\033[1;32m║ ➣ Support: https://bio.link/huu_vinh
\033[1;32m║ ➢ V4 👾 
\033[1;32m╚════════════════════════════════════════════════════════════════════════════════╝
""")
def banner():
...
def run(sdt,i):                  
  ...
  threading.submit(KILO,sdt)
  threading.submit(GAPO,sdt) 
  print("\033[91m[\033[1;32mSocial Network Express\033[91m] \033[1;32mSend SMS - Call | Delay : 5 | Trạng Thái : Thành Công ",)  
  for j in range(0, 5):
    print(f"\033[91m[\033[1;32mSocial Network Express\033[91m]\033[1;32mLOADING\r",end="")
